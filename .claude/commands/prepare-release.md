...
3. Run all formatting, linting, and type checking: `uv run ruff format && uv run ruff check --fix && uv run ty check && uv run pytest`
...
2. Create a draft release description based on the recent code changes and place it in `ai_working/draft_release_{version}.md`
