...
1. OpenAI instructions
   - Read the OpenAI models from two files to find any latest models
      - `.venv/lib/python3.11/site-packages/openai/types/shared/chat_model.py` - the `ChatModel` Literal
      - `.venv/lib/python3.11/site-packages/openai/types/shared_params/responses_model.py` - the additional models in the `ResponsesModel` Literal (excluding `str` and `ChatModel`)
...
2. Gemini instructions
   - Read the Gemini models from `.venv/lib/python3.11/site-packages/google/genai/_interactions/types/model.py`. Use these for `SupportedModelGemini`.
